...
class MLP:
    def __init__(self, x, n_in, n_hidden, n_out, h0_activation_f, out_activation_f, h0_W=None, h0_b=None, out_W=None, out_b=None):
        ...
        self.out_layer.forward_pass(self.h0_layer.a)    # compute hidden layer activation

        self.cost = self.out_layer.cost

        self.params = self.h0_layer.params + self.out_layer.params
    ...
    def errors(self, x, y):
        h0_a = self.h0_layer.prediction(x)
        return self.out_layer.errors(h0_a, y)
...
def steering_mlp(input_path, pickle_outpath):
    ...
    trainer = MLPTrainer(input_path, sensor_fields, action_fields, max_laps, n_hidden, n_out, h0_activation_f, out_activation_f)
    trainer.train(batch_size=600, max_epochs=200, L1_reg=0.01, L2_reg=0.01)
    trainer.save_pickles(pickle_outpath, sensor_fields, action_fields)
    trainer.save_text(pickle_outpath, sensor_fields)
