...
class MLP:
    def __init__(self, x, n_in, n_hidden, n_out, h0_activation_f, out_activation_f, h0_W=None, h0_b=None, out_W=None, out_b=None):
        self.n_hidden = n_hidden
        self.h0_layer = Hidden_layer(n_in, n_hidden, h0_activation_f, W=h0_W, b=h0_b)
        self.h0_layer.forward_pass(x)   # compute hidden layer activation

        #-----
        self.h1_layer = Hidden_layer(n_hidden, n_hidden, h0_activation_f, W=h0_W, b=h0_b) #TODO: remove
        self.h1_layer.forward_pass(self.h0_layer.a)   # compute hidden layer activation #TODO: remove

        self.out_layer = Hidden_layer(n_hidden, n_out, out_activation_f, W=out_W, b=out_b)
        # self.out_layer.forward_pass(self.h0_layer.a)    # compute hidden layer activation #TODO: uncomment

        #----
        self.out_layer.forward_pass(self.h1_layer.a)    # compute hidden layer activation   #TODO: remove

        self.cost = self.out_layer.cost

        # self.params = self.h0_layer.params + self.out_layer.params    #TODO: uncomment
        self.params = self.h0_layer.params + self.h1_layer.params + self.out_layer.params #TODO: remove
        # self.learning_rate = learning_rate

        self.L1 = (
            abs(self.h0_layer.W).sum()
            + abs(self.out_layer.W).sum()
            + abs(self.h1_layer.W).sum() #TODO: remove
        )

        self.L2 = (
            (self.h0_layer.W ** 2).sum()
            + (self.out_layer.W ** 2).sum()
            + (self.h1_layer.W ** 2).sum() #TODO: remove
        )

    def predict(self, x):
        h0_a = self.h0_layer.prediction(x)
        a = self.out_layer.prediction(h0_a)

        return a

    def errors(self, x, y):
        h0_a = self.h0_layer.prediction(x)
        h1_a = self.h1_layer.prediction(h0_a) #TODO: remove
        return self.out_layer.errors(h1_a, y) #TODO: remove
        # return self.out_layer.errors(h0_a, y) #TODO: uncomment
...
class MLPTrainer():
    ...
    def train(self, learning_rate=0.01, batch_size=600, max_epochs=100,
        ...
        self.network = MLP(x, n_in, self.n_hidden, self.n_out, self.h0_activation_f, self.out_activation_f)
    ...
    def save_pickles(self, pickle_outpath, sensor_fields, action_fields):
        ...
        pickles['activation_h0'] = self.h0_activation_f_name
        pickles['activation_out'] = self.out_activation_f_name
        pickles['scaler'] = self.scaler
        pickles['scaler_means'] = self.scaler.mean_
        pickles['scaler_stds'] = self.scaler.std_
        pickles['h0_W'] = self.network.h0_layer.W.get_value()
        pickles['h0_b'] = self.network.h0_layer.b.get_value()
        pickles['h1_W'] = self.network.h1_layer.W.get_value()
        pickles['h1_b'] = self.network.h1_layer.b.get_value()
    ...
    def save_text(self, text_outpath, sensor_fields):
        w_hidden_in = np.array(self.network.h0_layer.W.get_value(), dtype=float)
        w_hidden_in = np.vstack((w_hidden_in, self.network.h0_layer.b.get_value()))
        ...
        with open(filename, 'w') as f:
            ...
            f.write(self.h0_activation_f_name+'\n\n')
            ...
            self.write_matrix_values(f, w_hidden_in)
...
def acceleration_mlp(input_path, pickle_outpath):
    ...
    h0_activation_f = T.nnet.relu
    out_activation_f = T.nnet.relu

    trainer = MLPTrainer(input_path, sensor_fields, action_fields, max_laps, n_hidden, n_out, h0_activation_f, out_activation_f)
    trainer.train(batch_size=600, max_epochs=200, L1_reg=0.001, L2_reg=0.001)
...
if __name__=='__main__':
    ...
    # gear_mlp(input_path, pickle_outpath+'brake'+identifier)
